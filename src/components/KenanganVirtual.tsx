...
const KenanganVirtual: React.FC<PanoProps> = ({
  dataKenanganVirtual,
  place,
  lang,
}) => {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (container) {
        ...
        // POTO1
        container.createHotspot(
          picturespot1.current,
          { yaw: 1.1165199114658257, pitch: 0.009338294282953186 },
          { perspective: { radius: 3175, extraTransforms: "rotateY(-25deg)" } }
        );

        // POTO2
        container.createHotspot(
          picturespot2.current,
          { yaw: 2.101661332410651, pitch: 0.008970128824859813 },
          { perspective: { radius: 3350, extraTransforms: "rotateY(30deg)" } }
        );

        // POTO3
        container.createHotspot(
          picturespot3.current,
          { yaw: 2.601822736805576, pitch: 0.009388910134699557 },
          { perspective: { radius: 3325, extraTransforms: "rotateY(-30deg)" } }
        );

        // POTO4
        container.createHotspot(
          picturespot4.current,
          { yaw: -2.696497999467635, pitch: 0.009920743633978546 },
          { perspective: { radius: 3200, extraTransforms: "rotateY(25deg)" } }
        );

        // POTO5
        container.createHotspot(
          picturespot5.current,
          { yaw: -2.1022758458091104, pitch: 0.008922370526379453 },
          { perspective: { radius: 3325, extraTransforms: "rotateY(-30deg)" } }
        );

        // POTO6
        container.createHotspot(
          picturespot6.current,
          { yaw: -1.117538709593365, pitch: 0.010069377139673819 },
          { perspective: { radius: 3225, extraTransforms: "rotateY(25deg)" } }
        );

        // INFOSPOT POTO1
        container.createHotspot(textInfo.current, {
          yaw: 1.2663669301631675,
          pitch: -0.1802312721318824,
        });

        // LINK HOTSPOT TO MENU
        container.createHotspot(menuGallery.current, {
          yaw: 5.6887631231131556,
  ...
  return (
    <>
      ...
      <div
        ref={iframespot}
        id="iframespot"
        className="relative w-[576px] h-[324px]"
      >
        {/* <iframe id="youtube"
      ...
      <>
        <div
          onClick={() =>
            router.push(
              `/${lang}/${wedding.wedding_slug}/${guest.guest_slug}/kenanganvirtual/gallery`
            )
          }
          ref={menuGallery}
        ...
        >
          <img
            src="/assets/kenanganVirtual/link.png"
            alt=""
            className="link-hotspot-icon"
          />
          <div className="hotspot-tooltip link-hotspot-tooltip">Menu</div>
        </div>
        ...
        <div id="textInfo" ref={textInfo}>
          ...
          <div className="tooltip-content">
            <p>
              Dulu, anak SMA pemalu, Agy, naksir gadis populer, Yoriko. Saat
              kuliah, dia berjuang keras, berprestasi, dan berhasil memenangkan
              hati Yoriko. Cinta mereka mekar setelah melewati lika-liku karir
              kuliah.
            </p>
          </div>
        </div>
