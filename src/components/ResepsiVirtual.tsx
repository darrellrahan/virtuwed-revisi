...
const ResepsiVirtual: React.FC<PanoProps> = ({ dataPano, lang }) => {
  ...
  const handleLihatKV = () => {
    router.push(
      `/${lang}/${wedding.wedding_slug}/${guest.guest_slug}/menu/kenanganvirtual?place=panoScenes[0]`
    );
  };
  ...
  const handleKonfirmasi = async () => {
    ...
    try {
      if (digitalGift != gifts[5]) {
        const postGift = await axios.post(
          "https://panel.virtuwed.id/api/gift",
          {
    ...
    if (typeof window !== "undefined") {
      ...
      const backButtonLogic = () => {
        ...
        switch (tempattempat) {
          ...
          case "Back.webp":
            ...
            panoScenes[1].scene.lookTo(
              { yaw: 0, pitch: -0.056795042541741836 },
      ...
      const handleClicklookToHadiah = (e: Event) => {
        ...
        const lookToHadiah2Func = () => {
          if (
            berikanHadiah.current &&
            berikanHadiah.current?.classList.contains("invisible")
          ) {
            berikanHadiah.current?.classList.remove("invisible");
          }
          modalUcapanSelamat.onClose();
          panoScenes[1].scene.lookTo(
            { yaw: 1.4731802513717511, pitch: 0.17922631245596676 },
            options
          );
      ...
      const checklookToHadiah = () => {
        setTimeout(() => {
          lookToHadiah.current?.addEventListener(
            "click",
            handleClicklookToHadiah
          );
          lookToHadiah.current?.addEventListener(
            "touchstart",
            handleClicklookToHadiah
      ...
      const handleClicklookToKonfirmasi = (e: Event) => {
        ...
        const lookToKonfirmasi2Func = () => {
          modalHadiah.onClose();
          panoScenes[1].scene.lookTo(
            { yaw: -3.1149072553601655, pitch: -0.0070584653244445406 },
            options
          );

          setTimeout(function () {
            switchScene(panoScenes[2]);
            panoScenes[2].scene.lookTo(
              { pitch: -0.1851510231312865, yaw: 3.1373107204237645 },
              { transitionDuration: 0 }
            );

            setTimeout(function () {
              panoScenes[2].scene.lookTo(
                { yaw: 1.6829013691100227, pitch: 0.21238892003715293 },
                options
              );
              setTimeout(function () {
                modalKonfirmasi.onOpen();
        ...
        setTimeout(() => {
          lookToKonfirmasi2.current?.addEventListener(
            "click",
            lookToKonfirmasi2Func
          );
        }, 100);
      };
      ...
      const checklookToKonfirmasi = () => {
        setTimeout(() => {
          lookToKonfirmasi.current?.addEventListener(
            "click",
            handleClicklookToKonfirmasi
          );
          lookToKonfirmasi.current?.addEventListener(
            "touchstart",
            handleClicklookToKonfirmasi
        ...
        setTimeout(() => {
          lookToEnd.current?.addEventListener("click", async function () {
            try {
              ...
              if (
                keluarResepsi.current &&
                keluarResepsi.current?.classList.contains("invisible")
              ) {
                keluarResepsi.current?.classList.remove("invisible");
              }
              if (
                lihatKenanganVirtual.current &&
                lihatKenanganVirtual.current?.classList.contains("invisible")
              ) {
                lihatKenanganVirtual.current?.classList.remove("invisible");
              }
              panoScenes[2].scene.lookTo(
                { yaw: 3.1373107204237645, pitch: -0.1851510231312865 },
                options
              );
      ...
      berikanHadiah.current?.addEventListener(
        "touchstart",
        checklookToKonfirmasi
      );
      ...
      if (containerKoridor) {
        containerKoridor.createHotspot(
          bridePicture1.current,
          { yaw: -1.164598164451725, pitch: -0.3047599598934614 },
          {
            perspective: {
              radius: 2080,
              extraTransforms: "rotateY(25deg) rotateX(15deg) rotateZ(7deg)",
            },
          }
        );
        containerKoridor.createHotspot(
          bridePicture2.current,
          { yaw: -0.7283039004229686, pitch: -0.06379271830326516 },
          {
            perspective: {
              radius: 3830,
              extraTransforms: "rotateY(46deg) rotateX(3deg) rotateZ(3deg)",
            },
          }
        );
        containerKoridor.createHotspot(
          bridePicture3.current,
          { yaw: -0.38660091738350033, pitch: 0.0854400652578704 },
          {
            perspective: {
              radius: 4550,
              extraTransforms: "rotateY(7deg) rotateX(12deg) rotateZ(-0.75deg)",
            },
          }
        );
        containerKoridor.createHotspot(
          bridePicture4.current,
          { yaw: 0.16648722780547587, pitch: 0.04460063328119723 },
          {
            perspective: {
              radius: 8850,
              extraTransforms: "rotateY(-45deg) rotateX(12deg) rotateZ(2deg)",
            },
          }
        );
        containerKoridor.createHotspot(
          bridePicture5.current,
          { yaw: 0.3852822083866734, pitch: 0.08653882466922802 },
          {
            perspective: {
              radius: 4550,
              extraTransforms: "rotateY(-7deg) rotateX(12deg) rotateZ(0.75deg)",
        ...
        containerKoridor.createHotspot(
          bridePicture6.current,
          { yaw: 0.7285051078120084, pitch: -0.06574146003724834 },
          {
            perspective: {
              radius: 3830,
              extraTransforms: "rotateY(-46deg) rotateX(3deg) rotateZ(-3deg)",
        ...
        containerKoridor.createHotspot(
          bridePicture7.current,
          { yaw: 1.1642880111712266, pitch: -0.30463877068957146 },
          {
            perspective: {
              radius: 2080,
              extraTransforms: "rotateY(-25deg) rotateX(15deg) rotateZ(-7deg)",
      ...
      if (containerFront) {
        containerFront.createHotspot(
          ucapkanSelamat.current,
          { yaw: 0, pitch: -0.056795042541741836 },
        ...
        containerFront.createHotspot(
          bridePicture8.current,
          { yaw: 2.7664720145284134, pitch: -0.13936477893798127 },
          {
            perspective: {
              radius: 9000,
              extraTransforms: "rotateY(-18deg) rotateX(8deg) rotateZ(-3deg)",
            },
          }
        );
        containerFront.createHotspot(
          bridePicture9.current,
          { yaw: 2.896924155498631, pitch: -0.1448877727448732 },
          {
            perspective: {
              radius: 8700,
              extraTransforms: "rotateY(-12deg) rotateX(8deg) rotateZ(-2deg)",
            },
          }
        );
        containerFront.createHotspot(
          bridePicture10.current,
          { yaw: -2.898176835461687, pitch: -0.1445024049846424 },
          {
            perspective: {
              radius: 8700,
              extraTransforms: "rotateY(12deg) rotateX(8deg) rotateZ(2deg)",
        ...
        containerFront.createHotspot(
          bridePicture11.current,
          { yaw: -2.767850265539211, pitch: -0.13883335444879563 },
          {
            perspective: {
              radius: 9000,
              extraTransforms: "rotateY(18deg) rotateX(8deg) rotateZ(3deg)",
      ...
      if (containerBack) {
        ...
        containerBack.createHotspot(
          keluarResepsi.current,
          { yaw: 3.1409142956927933, pitch: 0.004879153410453085 },
          { perspective: { radius: 300, extraTransforms: "rotateY(0deg)" } }
        );

        containerBack.createHotspot(
          bridePicture12.current,
          { yaw: 0.7700412269389645, pitch: 0.17738112076748713 },
          {
            perspective: {
              radius: 2200,
              extraTransforms: "rotateY(-16deg) rotateX(6deg) rotateZ(3deg)",
            },
          }
        );

        containerBack.createHotspot(
          bridePicture13.current,
          { yaw: 2.0441387920684173, pitch: -0.3269953170850819 },
          {
            perspective: {
              radius: 3950,
              extraTransforms: "rotateY(-64deg) rotateX(8deg) rotateZ(-16deg)",
            },
          }
        );

        containerBack.createHotspot(
          bridePicture14.current,
          { yaw: 2.250330714266445, pitch: -0.43670196647323856 },
          {
            perspective: {
              radius: 2980,
              extraTransforms:
                "rotateY(-53.5deg) rotateX(15deg) rotateZ(-19.5deg)",
            },
          }
        );

        containerBack.createHotspot(
          bridePicture15.current,
          { yaw: 2.739585960629082, pitch: -0.037810082463801464 },
          {
            perspective: {
              radius: 6650,
              extraTransforms: "rotateY(65.5deg) rotateX(0.5deg) rotateZ(2deg)",
            },
          }
        );

        containerBack.createHotspot(
          bridePicture16.current,
          { yaw: -2.7404817812359674, pitch: -0.0385358756343539 },
          {
            perspective: {
              radius: 6650,
              extraTransforms:
                "rotateY(-65.5deg) rotateX(0.5deg) rotateZ(-2deg)",
            },
          }
        );

        containerBack.createHotspot(
          bridePicture17.current,
          { yaw: -2.250996098602439, pitch: -0.4365461346165951 },
          {
            perspective: {
              radius: 2980,
              extraTransforms:
        ...
        containerBack.createHotspot(
          bridePicture18.current,
          { yaw: -2.045404274690286, pitch: -0.3277844486449055 },
          {
            perspective: {
              radius: 3950,
              extraTransforms: "rotateY(64deg) rotateX(8deg) rotateZ(16deg)",
      ...
      return () => {
        ...
        ucapkanSelamat.current?.removeEventListener(
          "touchstart",
          checklookToHadiah
        );
        lookToHadiah.current?.removeEventListener(
          "click",
          handleClicklookToHadiah
        );
        lookToHadiah.current?.removeEventListener(
        ...
        berikanHadiah.current?.removeEventListener(
          "click",
          checklookToKonfirmasi
        );
        berikanHadiah.current?.removeEventListener(
        ...
        checkout.current?.removeEventListener(
          "touchstart",
          handleClickCheckout
        );
  ...
  return (
    <>
      ...
      <div
        className="light h-full w-full absolute saturate-0 transition-all duration-1000 delay-[3000ms]"
        ref={panoRef}
      ></div>
      ...
      <div
        ref={opening}
        className="transition-opacity duration-500 opacity-100 absolute w-full h-full z-50 hidden"
        style={{
          maxWidth: "100%",
          maxHeight: "100%",
          minWidth: "200px",
      ...
      <div
        ref={openingTouch}
        id="openingTouch"
        className="hidden pointer-events-none absolute w-full h-full z-40"
        style={{
          maxWidth: "400px",
          maxHeight: "300px",
          minWidth: "200px",
      ...
      >
        <div className="relative overflow-auto w-full h-full grid text-center justify-items-center content-center p-6 ">
          ...
          />
          <h4 className="font-deAetna text-xl text-white lg:text-2xl">
        ...
        <div className="bg-White py-3 rounded-lg max-w-lg mx-auto">
          <ButtonGroup className="w-full !h-full" size="lg">
            <Button
              ...
              startContent={<i className="ri-arrow-left-s-line ri-xl"></i>}
              className="w-full text-N800 bg-White"
            >
              <p className="l3-r font-deAetna">Kembali</p>
            </Button>
            <Button
              radius="sm"
              className="!rounded !h-full gap-1.5 grid grid-flow-row w-full bg-gradient-to-r from-primaryGradient-start to-primaryGradient-end text-white py-2"
              onPress={modalLivestream.onOpen}
            >
            ...
            <Button
              ...
              endContent={<i className="ri-arrow-right-s-line ri-xl"></i>}
              className="w-full text-N800 bg-White"
            >
              ...
              <ModalBody className="p-0 m-0">
                <div className="w-full h-full relative">
                  ...
                  <div className="bg-dark flex gap-x-2.5 justify-items-center justify-center items-center w-full h-14 fixed top-0 left-0">
                    ...
                    <Button
                      className="z-10 absolute left-2 flex justify-self-start"
                      onPress={modalLivestream.onClose}
                      isIconOnly
                      color="secondary"
                      variant="light"
                      aria-label="Close"
                  ...
                  <YouTubePlayer
                    ...
                    videoId={
                      wedding.resepsi_virtual.wedding_live_streaming_link.query
                        ? wedding.resepsi_virtual.wedding_live_streaming_link
                            .query
                        : "HqYhkpGgZXc"
                    }
                  />
                  ...
                  <div className="fixed b w-full bottom-0 left-0 bg-dark py-2.5 px-3 flex gap-x-2.5">
                    ...
                    <button
                      onClick={() =>
                        document
                          .querySelector("#digitalGiftContainer2")
                  ...
                  <div
                    id="digitalGiftContainer2"
                    className="absolute w-full overflow-y-auto px-6 z-10 hidden bg-red-600"
                  >
                    ...
                    <div className="text-center text-black bg-White w-full grid gap-3 px-3 py-4 rounded">
                      ...
                      <div className="grid justify-items-center text-center w-full max-w-[500px] mx-auto">
                        <h3 className="capitalize text-N800">
                          berikan hadiah digital
                        </h3>
                        <p className="p3-r text-N700 max-w-[285px]">
                          Hadiah digital hanya merupakan simbolis, pengantin
                          akan menerima nominal dari hadiah yang anda berikan
                        </p>
                      </div>
                      ...
                      <div className="p-3 grid w-full max-w-[500px] mx-auto">
                        ...
                        >
                                ...
                                className={({ active, checked }) =>
                                  `${
                                    checked
                                      ...
                                      : "border-none"
                                  } grid w-full h-36 rounded-md items-end p-1.5 cursor-pointer`
                                }
                              >
                                <div className="grid justify-start text-white text-start">
                                  <p className="l4-r font-deAetna capitalize">
                                    {gift.name}
                                  </p>
                          ...
                          <div className="flex w-full items-center gap-1 text-red-500">
                            ...
                            <p className="p3-r">
                              Invoice akan dikirimkan ke whatsapp anda
                            </p>
                          </div>
                        </RadioGroup>
                        <Button
                          className="rounded mt-3 z-10"
                          color="secondary"
                          startContent={<i className="ri-gift-line ri-xl"></i>}
                        >
                      ...
                      {/* DECORATION */}
                      <Image
                        src={"/assets/virtuwed/accent/vintage-ornaments.png"}
                        alt="decoration"
                        width={110}
                        height={110}
      ...
      <div ref={bridePicture1} className="relative w-[1024px] h-[768px]">
        <Image
          src={
            ...
            wedding.media.prewedding_photos[0]
          }
      ...
      <div ref={bridePicture2} className="relative w-[1350px] h-[1080px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[1]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture3} className="relative h-[1024px] w-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[2]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture4} className="relative h-[1024px] w-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[3]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture5} className="relative h-[1024px] w-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[4]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture6} className="relative w-[1350px] h-[1080px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[5]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture7} className="relative w-[1024px] h-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[6]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={ucapkanSelamat}>
        <Button
          startContent={<i className="ri-message-2-line ri-lg"></i>}
          color="secondary"
          className="rounded"
          onPress={modalUcapanSelamat.onOpen}
        >
      ...
      <div ref={berikanHadiah} className="invisible">
        <Button
          startContent={<i className="ri-gift-line ri-lg"></i>}
          color="secondary"
          className="rounded"
          onPress={modalHadiah.onOpen}
        >
      ...
      <Modal
        className="light light:text-black mx-6 bg-White"
            ...
            <>
              <ModalHeader>
                <div className="text-center w-full">
                  <h3 className="text-N800 capitalize">
                    berikan ucapan selamat
                  </h3>
                  <p className="text-N700 p3-r">
                    Anda dapat memberikan ucapan melalui video ataupun foto
                  </p>
                </div>
              </ModalHeader>
              <ModalBody>
                <div className="grid gap-3 w-full border border-tertiary rounded-b pt-6 px-3 pb-3 overflow-x-hidden">
                  <div className="grid gap-1">
                    <p className="l3-r text-N700 font-deAetna">
                      Upload Foto/Video Ucapan
                    </p>
                    ...
                    >
                      <div className="bg-white grid gap-1 justify-items-center border border-N300 rounded-md py-4 px-3 text-N400 cursor-pointer">
                        ...
                        <p className="p2-r">
                          Klik untuk {file ? "ganti" : "upload"} foto/video
                        </p>
                      </div>
                    </FileUploader>
                  ...
                  <div className="grid gap-1">
                    ...
                    <form className="grid gap-6">
                      <textarea
                        className="resize-y appearance-none rounded-md w-full p-3 text-N800 leading-tight border border-N300 focus:outline-none focus:shadow-outline"
                        placeholder="..."
                        rows={3}
                        value={ucapanSelamat}
                        onChange={(e) => setUcapanSelamat(e.target.value)}
                      />
                    </form>
                  </div>
                  ...
                  <Button
                    ref={lookToHadiah}
                    id="lookToHadiah"
                    onPress={modalInformativeUcapanSelamat.onOpen}
                    className="rounded"
                    color="secondary"
                    startContent={<i className="ri-send-plane-line ri-xl"></i>}
                  >
            ...
            <>
              <ModalBody className="grid justify-items-center">
                ...
                <div className="grid gap-2 justify-items-center text-center">
                  ...
                  <h4 className="text-N800">
                    {file != undefined
                      ...
                      : ucapanSelamat != ""
                      ? "Yakin Ga Sekalian Kirim Foto/Video?"
                  ...
                  <p className="p3-r text-N600">
                    {file != undefined
                      ...
                      : ucapanSelamat != ""
                      ? "Kirim Foto/Video ucapan anda untuk pengantin"
                      : "Ucapan anda akan sangat berharga bagi momen spesial kami"}
                  </p>
                </div>
                ...
                <form className="flex gap-1 w-full" method="dialog">
                  ...
                  <Button
                    startContent={<i className="ri-message-3-line ri-xl" />}
                    className="rounded w-full bg-transparent text-tertiary border-tertiary border"
                    onPress={onClose}
                  >
      ...
      {/* MODAL HADIAH */}
      <Modal
        className="h-[600px] overflow-y-scroll py-4"
        placement="top-center"
            ...
            <>
              <ModalHeader>
                <div className="text-center w-full">
                  ...
                  <p className="text-N700 p3-r">
                    Hadiah digital hanya merupakan simbolis, pengantin akan
                    menerima nominal dari hadiah yang anda berikan
                  </p>
                </div>
              </ModalHeader>
              <ModalBody className="grid justify-items-center">
                ...
                <div className="p-3 grid w-full max-w-[500px] mx-auto">
                  ...
                  >
                    ...
                    <RadioGroup.Option
                      ...
                      className={({ active, checked }) =>
                        `${
                          checked ? "bg-tertiary text-White" : "text-tertiary"
                        } flex w-full border border-tertiary px-3 rounded items-center py-3 relative cursor-pointer`
                      }
                    >
                      <i className="ri-message-3-line ri-xl absolute"></i>
                      <p className="capitalize w-full text-center font-deAetna l3-r">
                        {gifts[5].name}
                      </p>
                    </RadioGroup.Option>
                  ...
                  <Button
                    ref={lookToKonfirmasi}
                    className="rounded mt-3"
                    color="secondary"
                    startContent={<i className="ri-gift-line ri-xl" />}
                    onPress={modalInformativeHadiah.onOpen}
                  >
                    <p className="l2-r font-deAetna">kirim hadiah</p>
                  </Button>
                </div>
            ...
            <>
              <ModalBody className="grid justify-items-center">
                ...
                <form className="flex gap-1 w-full" method="dialog">
                  ...
                  >
                    <p className="l2-r font-deAetna">
                      {digitalGift === gifts[5] ? "Yakin" : "Lanjut"}
                    </p>
                  </Button>
                </form>
              </ModalBody>
      ...
      <div ref={bridePicture8} className="relative h-[1024px] w-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[7]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture9} className="relative h-[1024px] w-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[8]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture10} className="relative h-[1024px] w-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[9]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={bridePicture11} className="relative h-[1024px] w-[768px]">
        <Image
          src={
            `https://sgp1.vultrobjects.com/virtuwed-storage/` +
            wedding.media.prewedding_photos[10]
          }
          alt="Moment pengantin"
          className="object-cover object-center min-w-full w-full h-full"
          width={500}
      ...
      <div ref={checkout}>
        <Button
          startContent={<i className="ri-checkbox-line ri-lg"></i>}
          color="secondary"
          className="rounded"
          onPress={modalKonfirmasi.onOpen}
        >
            ...
            <>
              <ModalHeader>
                <div className="text-center w-full">
                  ...
                  <p className="text-N700 p3-r w-full">
                    Tolong konfirmasi ucapan selamat dan hadiah yang akan anda
                    berikan
                  </p>
                </div>
              </ModalHeader>
              <ModalBody>
                <div className="grid border border-tertiary rounded-lg p-3 gap-5">
                  ...
                  <div className="grid gap-1">
                    <p className="l3-r text-N700 font-deAetna">
                      <span className="font-bold font-amiamie">
                        {guest.nama}{" "}
                      </span>{" "}
                      memberi {wedding.wedding_name} sebuah...
                    </p>
                    {digitalGift != gifts[5] ? (
                      <div className="flex gap-1">
                        ...
                        <div className="grid gap-0.5 p-1.5 w-full content-center">
                          <p className="l2-r font-deAetna text-N700 capitalize">
                            {digitalGift.name}
                          </p>
                          <p className="p3-r text-N600 capitalize">
                            Rp. {digitalGift.price}
                          </p>
                        </div>
                      </div>
                    ) : (
                  ...
                  {/* UCAPAN */}
                  <div className="grid gap-1">
                    <p className="L3-r font-deAetna text-N700">
                      dan Ucapan Selamat...
                    </p>
                  ...
                  <div className="flex w-full items-center gap-1 text-red-500">
                    <i className="ri-error-warning-line self-start"></i>
                    <p className="p3-r">
                      Data anda akan dilihat oleh pengantin
                    </p>
                  </div>
                  ...
                  <div className="grid gap-2">
                    ...
                    {loading ? (
                      ...
                      <Button
                        ...
                        className="rounded"
                        color="secondary"
                        startContent={<i className="ri-check-line ri-xl"></i>}
                      >
            ...
            <>
              <ModalBody className="grid justify-items-center">
                ...
                <div className="grid gap-2 justify-items-center justify-center text-center">
                  ...
                  <h4 className="text-N800">Oh no!</h4>
                  <p className="p3-r text-N600">
      ...
      <div ref={lihatKenanganVirtual} className="invisible">
        <Button
          onPress={handleLihatKV}
          startContent={<i className="ri-eye-line ri-lg"></i>}
          color="secondary"
          className="rounded"
        >
      ...
      <div ref={keluarResepsi} className="invisible">
        <Button
          onPress={handleKeluarRV}
          startContent={<i className="ri-logout-box-r-line ri-lg"></i>}
          color="secondary"
          className="rounded"
        >
